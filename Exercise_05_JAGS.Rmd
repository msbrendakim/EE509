...
$$P(b) = N_p(b \vert b_0, V_b)$$ $$P(\sigma^2) = IG(\sigma^2 \vert s_1,s_2)$$
...
$$ y \sim N_n( Xb,S) $$ $$b \sim N_p(b_0, P_b)$$
...
```{r}
## Evaluate the MCMC chain for convergence, including relevant diagnostics and plots.
plot(jags.out)
gelman.diag(jags.out)
...
par(mfrow=c(1,3))
hist(as.matrix(jags.burn)[,"b[1]"], main="Posterior of b1", xlab="b1", col="lightpink")
hist(as.matrix(jags.burn)[,"b[2]"], main="Posterior of b2", xlab="b2", col="lightpink")
hist(as.matrix(jags.burn)[,"S"], main="Posterior of S", xlab="S", col="lightpink")

## Make a scatter plot of the data and add your posterior mean regression line
plot(x1, y, pch = 16, main = "Scatter Plot")
posterior_b <- colMeans(as.matrix(jags.burn))
abline(a = posterior_b[1], b = posterior_b[2], col = "red", lwd = 2)

## pairs plot and parameter correlation matrix
pairs(as.matrix(jags.burn))
cor(as.matrix(jags.burn))

## copare the summary statistics
summary(lm(y ~ x1))
```

```
Looking at the pairs plot, b[1] (intercept) and b[2] (slope) show that they are in a negative correlation. S (precision parameter) has no correlation with b[1] and b[2]. In correlation matrix, the coefficient of b[1] and b[2] is 0.89, which shows strong negative correlation between them. 

Looking at the summary statistics for the Bayesian regression model to those from the classical regression, the JAGS output shows the chains for b[2] (slope coefficient) which appears to have a mean around 2.01, very close to the classical estimate of 2.01369. 
```
...
model{
  ...
  for(i in 1:n){
	  mu[i] <- b[1] + b[2]*x1[i] + b[3]*x2[i] + b[4]*x1[i]*x2[i]
	  y[i]  ~ dnorm(mu[i],S)		        ## data model
  }
}
"
...
```{r}
## specify priors
data$b0 <- as.vector(c(0,0,0,0))      ## regression b means
data$Vb <- solve(diag(10000,4))   ## regression b precisions
data$s1 <- 0.1                    ## error prior
data$s2 <- 0.1                    ## error prior
```
...
inits <- list()
for(i in 1:nchain){
 inits[[i]] <- list(b = rnorm(4,0,5), S = runif(1,1/200,1/20))
}
```

```{r}
## running jags
j.model   <- jags.model (file = textConnection(multiple_regression),
                             data = data,
                             inits = inits,
                             n.chains = 3)
```
...
```{r}
## determine burnin period 
GBR_plot <- gelman.plot(jags.out)
```
...
```{r}
# check convergence after burnin
plot(jags.burn)
...
```{r}
## convert to matrix for additional analyses
out <- as.matrix(jags.burn)
```
...
par(mfrow=c(2,2))
hist(out[,"b[1]"], main="Posterior of b1 (Intercept)", xlab="b1", col="lightgreen")
hist(out[,"b[2]"], main="Posterior of b2 (x1 coef)", xlab="b2", col="lightgreen")
hist(out[,"b[3]"], main="Posterior of b3 (x2 coef)", xlab="b3", col="lightgreen")
hist(out[,"b[4]"], main="Posterior of b4 (interaction)", xlab="b4", col="lightgreen")
```

```{r}
## plot S
hist(out[,"S"], main="Posterior of S", xlab="S", col="lightgreen")
SD <- 1/sqrt(out[,"S"])
hist(SD, main="Posterior of Standard Deviation", xlab="SD", col="lightgreen")
```
