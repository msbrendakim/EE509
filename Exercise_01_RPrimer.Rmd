...
```
### g. How does R represent when the output of a function is not a number?
When the output of a function is not a number, R represent it as NaN.

### 2. Looking back at the Triginometry section, why are we multiplying the results of the atan and atan2 functions by 180/pi?
We are multiplying the results of the atan and atan2 functions by 180/pi since their output unit is radian. To get output with degree unit, we should multiply 180/pi to the radian value.

### 3. What is the difference between log and log10? (hint: use `help()`)
log calculates the natural logarithm ln, log base e, and log10 calculates log base 10.
...
### 6. Describe the difference in output between sum and cumsum.
sum calculates the total sum of all values in a vector, returning a single number. 
cumsum calculates the cumulative sum at each step, returning a vector of partial sums.

### 7.	Generate a sequence of even numbers from -6 to 6
```{r}
seq_even_num <- seq(-6, 6, by = 2)
seq_even_num
```

### 8.	Generate a sequence of values from -4.8 to -3.43 that is length 8 (show code)
```{r}
seq_values <- seq(-4.8, -3.43, length = 8)
seq_values
```
  - What is the difference between values in this sequence?
```{r}
difference <- seq_values[2] - seq_values[1]
difference
```
  
### 9. What is the sum of the exponential of the sequence in question 8?
```{r}
sum_exp <- sum(exp(seq_values))
sum_exp
```
...
```{r}
x = 1:10
write.table(x, "x_vector.csv", row.names = FALSE, col.names = FALSE, sep = ",")
```
...
### 15. Create a character vector that contains the names of 4 people you admire.
```{r}
people_admired = c("Taylor Swift", "Bill Gates", "Barack Obama", "Malala Yousafzai")
```
...
```{r}
tadpoles_3to5 <- dat[dat$tadpoles >= 3 & dat$tadpoles <= 5, ]
tadpoles_3to5
```
...
```{r}
daily_mean_temp <- met |>
  mutate(date = as.Date(time, tz = "GMT")) |> # convert from time to date
  group_by(date) |> 
  summarise(mean_air_temp = mean(AirTemp)) 

head(daily_mean_temp)
```
...
```{r}
blue_frogs <- dat$frogs[dat$color == "blue"]

hist(blue_frogs, xlab = "Number of Frogs", col = "blue")
```
...
```{r}
plot(met$ShortWave, met$RelativeHumidity)
```
...
met %>%
 filter(time >= "2013-06-04 00:00:00" & time <= "2013-08-27 00:00:00") %>%
 ggplot() +
  aes(x = AirTemp) +
  geom_histogram(bins = 30L, fill = "#FD2D95") +
  labs(
    x = "Air Temperature (°C)",
    y = "Frequency",
    title = "Histogram of Air Temperature",
    subtitle = "During Summer 2013"
  ) +
  theme_bw()
```
...
```{r}
red_frogs <- dat[dat$color == "red", ]

lm_red <- lm(tadpoles ~ frogs, data = red_frogs)
summary(lm_red)
```

####b.	Make a scatter plot of this data that includes the regression line
```{r}
plot(red_frogs$frogs, red_frogs$tadpoles, 
     main = "Scatter plot of red frogs' data", 
     xlab = "Number of Frogs", 
     ylab = "Number of Tadpoles", 
     col = "red", pch = 16)
abline(lm_red, lwd = 2)
```
...
```{r}
custom_plot <- function(data, x_col, y_col) {
  model <- lm(data[[y_col]] ~ data[[x_col]], data = data)
...
```{r}
subset_data <- dat[dat$tadpoles >= 5, ]
custom_plot(subset_data, "frogs", "tadpoles")
```
