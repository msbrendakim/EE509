...
plot(dia, cones, 
     ...
     ylab = "Mature (0/1)",
     pch = 16,
     main = "Tree Maturation vs Diameter")
...
# example probit function curve
xseq = seq(0,30,length=200)
lines(xseq,pnorm(xseq,b0,b1),type='l')
```
...
(1a) $$p(y_i = 1,2,\dotsc) = Bernoulli \left( m = 1 \vert \theta (x_i) \right) Pois \left( y_i \vert g(x_i) \right) $$
...
(1b) $$p(y_i = 0) = Bernoulli \left( m = 0 \vert \theta (x_i) \right) + Bernoulli \left( m = 1 \vert \theta (x_i) \right) Pois \left( 0 \vert g(x_i) \right) $$
...
```{r}
param  <- c(0.05,16,1)        ## initial guess (update this based on the first two models!)
...
```{r}
## two plots showing fecundity (dia vs ncone) and maturation (dia vs cones) showing both the raw data and the best fit model
...
# add fitted maturation curve theta(x)
xseq = seq(0, 30, length=200)
lines(xseq, pnorm(xseq, b0, b1), col="red", lwd=2)

# 2. fecundity plot
plot(dia, ncone,
     xlab = "Diameter (cm)",
     ylab = "Number of Cones",
     main = "Tree Fecundity vs Diameter",
     pch = 10)

# add pure fecundity curve g(x)
lines(xseq, a0 * xseq^2, col="blue", lwd=2)

# add combined fecundity and maturation curve $g(x) \cdot \theta(x)$
lines(xseq, a0 * xseq^2 * pnorm(xseq, b0, b1), col="red", lwd=2)

# legend
legend("topleft", 
       legend=c("Pure fecundity", "Expected cones"),
       col=c("blue", "red"),
       lwd=2)
```
...
```{r}
## Elevated CO2 data
dia.elev = b00$diam[tmt=="CO2"] 
ncone.elev = b00$cones[tmt=="CO2"] 

## fit the model for just the elevated CO2 treatement
out.elev = optim(param,
                 likfit,
                 method="L-BFGS-B",
                 lower=c(0.001,10,1),
                 upper=c(1,30,10),
...
## Save the output to the variable "out.elev" and store the same output values to the third element of the parameter vectors
a0[3] = out.elev$par[1]
b0[3] = out.elev$par[2]
b1[3] = out.elev$par[3]
...
## table of the values of the parameters
parameter_table = data.frame(
  Model = c("Full dataset", "Ambient CO2", "Elevated CO2"),
  a0 = c(a0[1], a0[2], a0[3]),
  b0 = c(b0[1], b0[2], b0[3]),
  b1 = c(b1[1], b1[2], b1[3]),
  n = c(n[1], n[2], n[3]),
  lnL = c(lnL[1], lnL[2], lnL[3])
)
...
# add 3 maturation curves
xseq = seq(0, 30, length=200)
# Full model
lines(xseq, pnorm(xseq, b0[1], b1[1]), col="black", lwd=2)
# Ambient CO2
lines(xseq, pnorm(xseq, b0[2], b1[2]), col="blue", lwd=2)
# Elevated CO2
lines(xseq, pnorm(xseq, b0[3], b1[3]), col="red", lwd=2)

legend("topleft", 
       legend=c("Full model", "Ambient CO2", "Elevated CO2"),
...
# 3 add combined fecundity curves (g(x)⋅θ(x))
# Full model
lines(xseq, a0[1] * xseq^2 * pnorm(xseq, b0[1], b1[1]), col="black", lwd=2)
# Ambient CO2
lines(xseq, a0[2] * xseq^2 * pnorm(xseq, b0[2], b1[2]), col="blue", lwd=2)
# Elevated CO2
lines(xseq, a0[3] * xseq^2 * pnorm(xseq, b0[3], b1[3]), col="red", lwd=2)
...
Yes, elevated CO2 have an effect on fecundity. The value of the parameter of elevated CO2 (a0 = 0.02677156) is higher than the parameter of ambient CO2 (a0 = 0.01880115), which means the tree grown with elevated CO2 would produce more cones. Also, looking at the fencundity graph, the red line (elevated CO2) consistently shows higher cone production than the blue line (ambient CO2) for trees of the same diameter.

2.  Is this effect simply due to there being larger trees in the elevated plots, or does CO2 affect the maturation rate and/or the fecundity of trees (i.e. the cone production at a given tree size)? If there is an effect, in what direction is it?

CO2 affect the maturation rate and fecundity of trees. Trees in elevated CO2 reach maturity at smaller diameters (b0 = 20.65 cm) compared to ambient CO2 (b0 = 23.64 cm), and the transition to maturity is sharper in elevated CO2 (b1 = 6.78 vs 8.48). Even when comparing trees of the same size, elevated CO2 trees produce more cones (higher a0 value). These indicate that CO2 level affects reproductive systems of trees beyond merely increasing tree size.
```
